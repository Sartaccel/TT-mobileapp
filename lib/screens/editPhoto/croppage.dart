...
class Croppage extends StatefulWidget {
  ...
  final bool shouldSaveDirectly;

  const Croppage({
    super.key,
    required this.imagePath,
    this.shouldSaveDirectly = false,
  });
...
class _CroppageState extends State<Croppage> {
  ...
  Future<void> _handleSave() async {
    ...
    if (widget.shouldSaveDirectly) {
      // Case: Save directly (when coming from EditPhotoPage)
      try {
        setState(() => isLoading = true);
        await uploadImage(_croppedImage!);
        Navigator.pop(
            context, _croppedImage!.path); // Return the saved image path
      } catch (e) {
        IconSnackBar.show(
          context,
      ...
      // Case: Return to EditPhotoPage (default behavior)
